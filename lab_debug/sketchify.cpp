...
RGBAPixel* myFavoriteColor(int blueval) {
    // Why did original method not work?
...
void sketchify(std::string inputFile, std::string outputFile) {
    // Load in.png
    // Must create a new PNG object, or will not be able to initalize
    ...
    for (unsigned y = 1; y < height-1; y++) {
        for (unsigned x = 1; x < width-1; x++) {
            ...
            if (score > 0.3) {
                // We are changing the address to map to a different address
