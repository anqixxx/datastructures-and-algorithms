...
template <typename T>
void BinaryTree<T>::printPaths(Node* subRoot, string path) const
{
  // Your code here
  if(!subRoot) return;
...
int BinaryTree<T>::sumDistances(const Node* subRoot, int depth) const
{
  ...
  int left_dist = sumDistances(subRoot->left, depth +1);
  int right_dist = sumDistances(subRoot->right, depth+1);
  // Your code here
  return depth + left_dist + right_dist;
}
