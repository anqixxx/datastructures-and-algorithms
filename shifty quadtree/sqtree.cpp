...
SQtree::Node * SQtree::buildTree(stats & s, pair<int,int> & ul,
  ...
  else if (split.first == ul.first){ // y line, horizontal spit, same width different heights
  ...
  else if (split.second == ul.second){ // x line, vertical split, same height different widths
...
SQtree::Node * SQtree::copyNode(Node * node){
  ...
  newNode->NW = copyNode(node->NW);  newNode->NE = copyNode(node->NE);
  newNode->SE = copyNode(node->SE);  newNode->SW = copyNode(node->SW);
  return newNode;
}
...
void SQtree::clearNode(Node *& node){
  if (node != NULL){
    clearNode(node->NE); clearNode(node->NW);
    clearNode(node->SE); clearNode(node->SW);
...
void SQtree::helpRender(Node * node, PNG & im) {
  ...
  if (node->NW == NULL && node->NE == NULL && node->SW == NULL && node->SE == NULL) { // no children
    ...
    helpRender(node->NW, im); helpRender(node->NE, im); // recursive calls
    helpRender(node->SW, im); helpRender(node->SE, im);
  }
}
...
bool SQtree:: validSplit(pair<int, int> ul, int x, int y, int w, int h){
  ...
  if (x ==(w+ul.first) || y == (h+ul.second)) return false; 
  else return true; 
}
