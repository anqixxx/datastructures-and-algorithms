...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::string upperword2 = word2;
    std::transform(upperword2.begin(), upperword2.end(), upperword2.begin(), ::toupper);
    if( dict.find(upperword1) !=  dict.end() &&  (dict.find(upperword2)) != dict.end())
        return dict.find(upperword1)->second == dict.find(upperword2)->second;
    return false;
}
