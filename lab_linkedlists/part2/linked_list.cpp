...
{
    if (head == NULL) return; // Base case, list is empty

    // Can do this recursively to traverse the linkedList
    if( head->next == NULL ){
        delete head;
        head = NULL; // Makes sure the that the previous element now points to NULL
    } else {
        delete_last_element(head->next); // Goes to next element until we reach the end
    }
}
...
{
    if (!head) return; // Base case, list is empty


    if (head->key == oldKey){ // If the element is the old key
        Node* temp = head; 
        head = head->next; // Make sure the element now points to the next element 
        // What exactly am I accessing?
        delete temp; // Want to delete the value of head, but head as a pointer must always point to the next in the list
        temp = NULL;
        // Therefore must create a new node called temp to delete head
        return; 
    } else{
        remove(head->next, oldKey);
    }

    // Instead can do this with only one base case
    // Take the first, to check if the list is empty
    // Otherwise, if the key is the old key
    // Create a temp variable to store the head
    // then use the head pointer to point to the next element instead
    // Then delete the temp variable to permanently delete the old key
    // Basically refine algorithm
}
...
void insert_after(Node *head, int oldKey, int newKey)
{
    ...
    if (head->key == oldKey){ // If the current element is the old key
        head->next = new Node(newKey, head->next); // Assign the element after to be a new node
        // that has the int value of the new key, and points to the element the current element
        // used to point to
    } 
    else{
...
Node *interleave(Node *list1, Node *list2)
{
    ...
    if (!list1){ // If one is empty, assign list 2 as list 1
        list1 = list2;
        list2 = NULL;
    }
    ...
    head->next = interleave(list2, list1->next); // Interleaves the list head until both pointers are null
    // This allows the current Node to point at the one that should be after it
