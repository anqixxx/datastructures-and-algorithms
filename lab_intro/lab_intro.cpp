...
PNG grayscale(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double adjusted_gre = 0.587 * pixel ->g;
      double adjusted_blu = .114 * pixel ->b;

      grayvalue = (adjusted_blu+adjusted_gre+adjusted_red)/3;
...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (distance <= 200){
        double scaledValue = 1-(distance*.005);

        // remember parathenses
        pixel->r = (unsigned char) (scaledValue * pixel->r);
        pixel->g = (unsigned char) (scaledValue * pixel->g);
        pixel->b = (unsigned char) (scaledValue * pixel->b);
      } 
      else{
...
PNG ubcify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      // If closer to blue, the distance is less
      if ( colordist(UBCBlue, *pixel ) <  colordist(UBCYellow, *pixel )){
        pixel->r = (unsigned char) 12;
        pixel->g = (unsigned char) 35;
        pixel->b = (unsigned char) 68;
        pixel->a = (unsigned char) 1.0;
...
PNG watermark(PNG firstImage, PNG secondImage) {
    for (unsigned x = 0; x < secondImage.width(); x++) {
      for (unsigned y = 0; y < secondImage.height(); y++) {
        RGBAPixel* pixelTwo = secondImage.getPixel(x, y);

        if (pixelTwo->r == 255 && pixelTwo->g == 255 && pixelTwo->b == 255){
          RGBAPixel* pixelOne = firstImage.getPixel(x, y);
          
          double red = (pixelOne->r + 40);
          double green = (pixelOne-> g + 40);
          double blue = (pixelOne-> b + 40);
          ...
          pixelOne->r = (unsigned char) red;
          pixelOne->g = (unsigned char) green;
          pixelOne->b = (unsigned char) blue;
        }
